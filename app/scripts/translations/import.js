...
csv()
  .fromFile(path.join(__dirname, '../../translations-import.csv'), {
    trim: true,
  })
  .then(importData => {
    const parsedData = parseData().filter(
      d => d[defaultLanguage] !== undefined,
    );

    for (const dataItem of importData) {
      const parsedItemIndex = parsedData.findIndex(
        d => d.name === dataItem.name,
      );
